...
export const createAuthor = async (data: Partial<Author>, t: TFunction<'translation', undefined>): Promise<Author> => {
  try {
    const existingAuthor = await authorRepository.findOne({ where: { fullname: data.fullname } });
    if (existingAuthor) {
...
export const updateAuthor = async (authorId: number, data: Partial<Author>, t: TFunction) => {
  try {
